...
class AppsController:
    ...
    def __init__(self):
        ...
        for contents_functions in self.contents_functions:
            self.register_contents_function(contents_functions)
    ...
    def register_contents_function(self, content_function):
        setattr(self, content_function, lambda: self.get_content(content_function))

    def get_content(self, content_function, separator="\n\n"):
        content = ""
    ...
    def load_apps_from_dir(self):
        for app_settings in config.apps:
            ...
            app_module = importlib.import_module(
                f"{config.apps_dir}.{app_settings['app']}"
            )
